function okay=EvalSPLpanel(figh, P, MaxSPL, MaxSPL2, MaxSPLfreq, Prefix, Xquerynames)
...
if any(P.SPL>MaxSPL),
    ...
    return;
elseif any(P.echoSPL>MaxSPL),
    any(P.echoSPL>MaxSPL),
    if Interactive, % report error and highlight SPL edit control
        GUImessage(figh,'SPL exceeds maximum attainable.', 'error', {[Prefix 'echoSPL'] Xquerynames{:}});
    end
    return;
end
