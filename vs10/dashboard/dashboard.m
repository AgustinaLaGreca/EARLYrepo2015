...
%   dashboard('launch') launches the Dashboard GUI for experiment control,
%   and returns a handle to the GUI. There can only be a single Dashboard
%   figure at a time. If a Dashboard GUI is already open, it receives
%   focus.
%
%   h=dashboard() returns a handle to the current dashboard GUI, or [] if
%   none is open.
%
...
%         figh = figh.Number; % removed by Jan (April 2018)
    ...
    case 'earcalib',
        ...
        local_GUImode(figh, 'busy');
        hstim = getGUIdata(figh, 'StimGUIhandle', nan);
        if ishandle(hstim), close(hstim); end
        hg = GUI(earcalib);
        waitfor(hg);
        local_GUImode(figh, 'ready');
