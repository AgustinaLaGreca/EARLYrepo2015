function varargout = EvalTHR_CORR(varargin)
%EVALTHR  evalauate dataset containing threshold curve and plot the
%correction
%   EVALTHR(ds) evaluates the threshold curve for dataset ds and adds the
%   correction calculated in correct_ds.

%   IT NEEDS TO BE COMPLETED by adding the fit calculation and
%   to the corrected curve as well.
%
...
%   use 'factory' as only input argument. The correction
%
%   See correct_ds
...
if (nargin == 1) && ischar(varargin{1}) && strcmpi(varargin{1}, 'factory')
    ...
    [ds, Param] = ParseArgs(DefParam, varargin{1:end-1});
    thr_corr = varargin{end};
end

%Calculate threshold curve ...
CalcData = CalcThrCurve(ds, Param, thr_corr);
...
%------------------------------------------------------------------
function CalcData = CalcThrCurve(ds, Param, thr_corr)
...
[CalcData.corr.cf, CalcData.corr.minthr, CalcData.corr.bwf, CalcData.corr.bw, ...
       CalcData.corr.q10] = ExtractUsefulParam(S.curve.freq, thr_corr, Param);

%Cut off tails if requested ...
if strcmpi(Param.tailcutoff, 'yes')
    [CalcData.thr.freq, CalcData.thr.thr, CalcData.corr.thr] = CutOffTails(CalcData.thr.freq, CalcData.thr.thr, thr_corr);
end    
...
%------------------------------------------------------------------
function [Freq, Thr, Thr_corr] = CutOffTails(Freq, Thr, Thr_corr)
...
%Reduce curve ...
[Freq, Thr, Thr_corr] = deal(Freq(StartIdx:EndIdx), Thr(StartIdx:EndIdx), Thr_corr(StartIdx:EndIdx));
...
if strcmpi(Param.xunit, 'hz') || (strcmpi(Param.xunit, 'auto') && (CalcData.thr.cf <= 1000))
    ...
    [Xorig_cor, Yorig_cor] = deal(CalcData.thr.freq, CalcData.corr.thr);
    if ~strcmpi(Param.fit, 'none')
        [Xfit, Yfit] = deal(CalcData.fit.freq, CalcData.fit.thr);
    end
    [CForig, BWforig] = deal(CalcData.thr.cf, CalcData.thr.bwf);
    [CForig_cor, BWforig_cor] = deal(CalcData.corr.cf, CalcData.corr.bwf);
    if ~strcmpi(Param.fit, 'none')
    ...
    [Xorig_cor, Yorig_cor] = deal(CalcData.thr.freq/1e3, CalcData.corr.thr);
    if ~strcmpi(Param.fit, 'none')
        [Xfit, Yfit] = deal(CalcData.fit.freq/1e3, CalcData.fit.thr);
    end
    [CForig, BWforig] = deal(CalcData.thr.cf/1e3, CalcData.thr.bwf/1e3);
    [CForig_cor, BWforig_cor] = deal(CalcData.corr.cf/1e3, CalcData.corr.bwf/1e3);
    if ~strcmpi(Param.fit, 'none')
...
if strcmpi(Param.fit, 'none')
    DataStr = {...
        ...
        sprintf('%.1fdB SPL @ %.0f%s Q%d = %.2f (CORRECT)', ...
    CalcData.corr.minthr, CForig_cor, FreqUnit, Param.thr, CalcData.corr.q10)};
else
    DataStr = { ...
        sprintf('%.1fdB SPL @ %.0f%s SR = %.2fsp/s Q%d = %.2f (ORIG)', ...
        CalcData.thr.minthr, CForig, FreqUnit, CalcData.thr.sr, Param.thr, CalcData.thr.q10); ...
...
if ~strcmpi(Param.fit, 'none')
    ...
    TmpX = denan([Xorig(:); Xorig_cor(:)]); TmpY = denan([Yorig(:); Yorig_cor(:)]);
end
...
line(Xorig_cor, Yorig_cor, 'LineStyle', ':', 'Marker', 'o', 'Color', 'r');
if ~strcmpi(Param.fit, 'none')
...
line(CForig_cor, CalcData.corr.minthr, 'LineStyle', 'none', 'Marker', 'o', 'Color', 'm');
if ~strcmpi(Param.fit, 'none')
    line(CFfit, CalcData.fit.minthr, 'LineStyle', 'none', 'Marker', '.', 'Color', 'k');
end
line(BWforig, CalcData.thr.minthr([1 1])+Param.thr, 'LineStyle', '-', ...
    'Marker', 'none', 'Color', 'r');
line(BWforig_cor, CalcData.corr.minthr([1 1])+Param.thr, 'LineStyle', '-', ...
    'Marker', 'none', 'Color', 'm');
if ~strcmpi(Param.fit, 'none')
